...
def test_empty_path_fails_closed():
    ...
    code, _ = run(stdin)
    expect("empty path fails closed", code, 2)


def test_missing_tool_input_fails_closed():
    stdin = json.dumps({})
    code, _ = run(stdin)
    expect("missing tool_input fails closed", code, 2)


def test_malformed_stdin_fails_closed():
    code, _ = run("not valid json")
    expect("malformed stdin fails closed", code, 2)
...
def test_no_stdin_fails_closed():
    ...
    if result.returncode != 2:
        ...
        sys.exit(1)
    print("  empty stdin fails closed: PASS (exit 2)")
